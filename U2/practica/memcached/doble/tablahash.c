 ...
 * Para manejar la colision vamos a aplicar Linear probing
 */
...
static void tablahash_redimensionar(TablaHash tabla) {
  tabla->capacidad *= 2;
  int* newElems = malloc(sizeof(void*) * tabla->capacidad);
  CasillaHash* aux = tabla->elems;
  tabla->elems = newElems;
  
  for (int i = 0; i < tabla->capacidad / 2; i++) {
    tablahash_insertar(tabla, aux->dato + i);
  }
